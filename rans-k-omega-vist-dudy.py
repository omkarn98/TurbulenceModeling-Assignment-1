...
folder = './'
filename = str(folder)+'model-vistnd-svr.bin'
model = load(str(folder)+'model-vistnd-svr.bin')
scaler_dudy = load(str(folder)+'scaler-dudy-svr.bin')
scaler_vist = load(str(folder)+'scaler-vist-svr.bin')
dudy_max, dudy_min = np.loadtxt(str(folder)+ 'min-max-dudy-svr.txt')
vist_max, vist_min = np.loadtxt(str(folder)+ 'min-max-vist-svr.txt')
...
vist_min_number = (vist < vist_min)
vist_max_number = (vist > vist_max)
...
vist_test = np.minimum(vist, vist_max)
vist_test = np.maximum(vist, vist_min)
...
vist_test=scaler_vist.fit_transform(vist_test)
...
plt.plot(uv_predict,yp,'g-',label="ML")
...
# cmu_arr = 1 * np.ones((98,1))

# fig1,ax1 = plt.subplots()
# plt.subplots_adjust(left=0.20,bottom=0.20)
# plt.plot(yplus, cmu_arr,'b-',label="CFD")
# plt.plot(yplus,cmu_predict,'r-',label="ML")
# # plt.plot(k_ML,yp,'g-',label="ML")
# plt.legend(loc="best",prop=dict(size=18))
# plt.xlabel(r'$C_\mu$')
# plt.ylabel('y')
